...
class PUClassifier:
    ...
    def loglikelihood(self, Xc, Xe, Y, w=1.):
        if self.da:
            ...
            # print(self.loglikelihood_lr(Xc, Xe, Y, w))
    ...
    def loglikelihood_da(self, Xc, Xe, Y, w=1.):
        ...
        - w: classification weights (default=1. else, should be an array of positive float with the same size as Y)
        '''
        # print(Y*(np.log(self.cmodel.pdf_pos(Xc)) + self.emodel.loge(Xe)) + (1-Y)*np.log(self.cmodel.pdf_pos(Xc)*(1-self.emodel.e(Xe)) + self.cmodel.pdf_neg(Xc)))
