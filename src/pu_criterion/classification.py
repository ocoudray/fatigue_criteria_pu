...
class LinearLogisticRegression(Classifier):
    ...
    def fit(self, Xc, gamma, w=1., warm_start=True):
        ...
        c1, c2 = 0.5*len(gamma)/sum(gamma), 0.5*len(gamma)/(len(gamma)-sum(gamma))
        w_ = np.concatenate([w*gamma, w*(1-gamma)], axis=0)
...
class LinearLogisticRegression2(Classifier):
    def __init__(self, C=1.):
        self.C = C
        self.model = LogisticRegression(penalty='l1', C=self.C, fit_intercept=False, warm_start=True, solver='liblinear')
        super(LinearLogisticRegression2, self).__init__()
...
class LinearDiscriminantClassifier(Classifier):
    ...
    def initialization(self, Xc, gamma, w=1.):
        self.params = {'pi':0.05*np.random.rand(), 'mu_0':np.random.randn(Xc.shape[1]), 'mu_1':np.random.randn(Xc.shape[1]), 'Sigma':1/len(Xc)*np.dot((Xc-Xc.mean(axis=0)).T, Xc-Xc.mean())}
        ...
        # mu0 = np.mean(Xc[gamma==0], axis=0)
        # mu1 = np.mean(Xc[gamma==1], axis=0)
        # pi = np.mean(gamma)
        # X0, X1 = np.dot((Xc-mu0).T,(1-gamma[:,np.newaxis])*(Xc-mu0)), np.dot((Xc-mu1).T,gamma[:,np.newaxis]*(Xc-mu1))
        # Sigma = 1/len(Xc)*(X0+X1)
        # self.params = {'pi':pi, 'mu_0':mu0, 'mu_1':mu1, 'Sigma':Sigma}
...
class RandomForestClass(Classifier):
    def __init__(self, max_depth=5):
        self.max_depth = max_depth
        super(RandomForestClass, self).__init__()
    ...
    def eta(self, Xc):
        try:
            return self.model.predict_proba(Xc)[:,1]
        except:
            return np.random.rand(Xc.shape[0])

    def logeta(self, Xc):
        try:
            return self.model.predict_log_proba(Xc)[:,1]
        except:
            return np.random.randn(Xc.shape[0])
